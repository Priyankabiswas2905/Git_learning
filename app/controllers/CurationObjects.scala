      ...
      case Some(cOld) => {
        spaces.get(cOld.space) match {
          case Some(s) => {
            ...
            val metadataSummary = metadatas.getMetadataSummary(ResourceRef(ResourceRef.curationObject, c.id), Some(cOld.space))

            val fileByDataset = curations.getCurationFiles(curations.getAllCurationFileIds(c.id))
            if (c.status != "In Preparation") {
              Ok(views.html.spaces.submittedCurationObject(c, fileByDataset, metadataSummary, metadatas.getDefinitions(metadataSummary.contextSpace), limit, s.name))
            } else {
              Ok(views.html.spaces.curationObject(c, metadataSummary, metadatas.getDefinitions(metadataSummary.contextSpace), limit))
            }
          }
          ...
          case _ => {
            curations.getCurationFolder(UUID(curationFolderId)) match {
              case Some(cf) => {
                ...
                val rdfMap = scala.collection.mutable.Map.empty[String, RdfMetadata]
                limitFileIds.map(f => { rdfMap(f.stringify) = metadatas.getMetadataSummary(ResourceRef(ResourceRef.curationFile, f), None) })
                val mDefs = metadatas.getDefinitions(rdfMap.head._2.contextSpace)
                ...
                Ok(views.html.curations.filesAndFolders(c, Some(cf.id.stringify), foldersList, folderHierarchy.reverse.toList, pageIndex, next, limitFileList.toList, rdfMap.toMap, mDefs))
              }
