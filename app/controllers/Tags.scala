 ...
 * Tagging.
 */
class Tags @Inject()(collections: CollectionService,
  ...
  def createTagList(user: Option[User]) = {
    ...
    if (indexService.isConnected) {
      val results = indexService.listTags()
      for ((k,v) <- results) {
