 ...
 * Tagging.
 */
class Tags @Inject()(collections: CollectionService, datasets: DatasetService, files: FileService,
  sections: SectionService, elasticsearchService: ElasticsearchService) extends SecuredController {
  ...
  def createTagList(user: Option[User]) = {
    ...
    if (current.configuration.getBoolean("elasticsearchSettings.enabled").getOrElse(false)) {
      val results = elasticsearchService.listTags()
      for ((k, v) <- results) {
  ...
  def computeTagWeights(user: Option[User]) = {
    ...
    if (current.configuration.getBoolean("").getOrElse(false)) {
      for ((k, v) <- elasticsearchService.listTags("dataset")) {
        weightedTags(k) = weightedTags(k) + v * current.configuration.getInt("tags.weight.dataset").getOrElse(1)
      }
      for ((k, v) <- elasticsearchService.listTags("file")) {
        weightedTags(k) = weightedTags(k) + v * current.configuration.getInt("tags.weight.files").getOrElse(1)
      }
      for ((k, v) <- elasticsearchService.listTags("section")) {
