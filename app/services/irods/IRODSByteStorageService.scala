 ...
 *
 */
class IRODSByteStorageService @Inject() (irodsService: IRODSService) extends ByteStorageService {
  /**
  ...
  def save(inputStream: InputStream, prefix: String): Option[(String, Long)] = {
      ...
      // combine all pieces
      val filePath = makePath(irodsService.userhome, prefix, relativePath)

      // make sure the connection is open
      if (!irodsService.conn) {
        irodsService.openIRODSConnection()
      }

      try {
        // create folder structure
        val file = irodsService.getFileFactory().instanceIRODSFile(filePath)
        ...
        val fos = irodsService.getFileFactory().instanceIRODSFileOutputStream(file)
        ...
        irodsService.closeIRODSConnection()
      }
  }
  ...
  def load(id: String, prefix: String): Option[InputStream] = {
    // combine all pieces
    val filePath = makePath(irodsService.userhome, prefix, id)
    ...
    try {
      val file = irodsService.getFileFactory().instanceIRODSFile(filePath)
      val is = irodsService.getFileFactory().instanceIRODSFileInputStream(file)
