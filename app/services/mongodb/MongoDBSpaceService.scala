      ...
      case Some(o) => {
        ...
        if (showAll) {
          ...
          user match {
            case Some(u) => {
              if(showPublic) {
                author ++ $or(statusFilter, ("_id" $in users.getUserSpaceAndRoleWithPermissions(u).filter(_.role.permissions.intersect(permissions.map(_.toString)).nonEmpty).map(x => new ObjectId(x.spaceId.stringify))))
              } else {
                author ++ $or(("_id" $in users.getUserSpaceAndRoleWithPermissions(u).filter(_.role.permissions.intersect(permissions.map(_.toString)).nonEmpty).map(x => new ObjectId(x.spaceId.stringify))))
              }
          ...
          user match {
            case Some(u) => {
              ...
              if (onlyTrial) {
                ...
                $or(author, statusFilter, ("_id" $in users.getUserSpaceAndRoleWithPermissions(u).filter(_.role.permissions.intersect(permissions.map(_.toString)).nonEmpty).filter((p: UserSpaceAndRole) =>
                  get(p.spaceId) match {
                    case Some(space) => {
                ...
                $or(author, statusFilter,  ("_id" $in users.getUserSpaceAndRoleWithPermissions(u).filter(_.role.permissions.intersect(permissions.map(_.toString)).nonEmpty).map(x => new ObjectId(x.spaceId.stringify))))
              } else {
                $or(author, ("_id" $in users.getUserSpaceAndRoleWithPermissions(u).filter(_.role.permissions.intersect(permissions.map(_.toString)).nonEmpty).map(x => new ObjectId(x.spaceId.stringify))))
